...
if command -v sh4-linux-gnu-as > /dev/null 2>&1; then
  for example in "$ROOT_DIR"/examples/*.cas "$BUILD_DIR/regress.cas"; do
    name=$(basename "$example" .cas)
    "$BUILD_DIR/commonasmc" "$example" --target superh -o "$BUILD_DIR/sh-${name}.s"
  ...
  sh4-linux-gnu-objdump -d "$BUILD_DIR/sh-probe.o"     | sed -n '/__commonasm_divmod/,/rts/p' | head -30
else
...
if command -v alpha-linux-gnu-as > /dev/null 2>&1; then
  for example in "$ROOT_DIR"/examples/*.cas "$BUILD_DIR/regress.cas"; do
    name=$(basename "$example" .cas)
    "$BUILD_DIR/commonasmc" "$example" --target alpha-gnu -o "$BUILD_DIR/alpha-${name}.s"
    alpha-linux-gnu-as -o "$BUILD_DIR/alpha-${name}.o" "$BUILD_DIR/alpha-${name}.s"
  done
  echo "the Alpha assembler accepted every alpha-gnu output."
else
  echo "no Alpha assembler found; skipped assembling the Alpha output."
fi

if command -v nios2-linux-gnu-as > /dev/null 2>&1; then
  for example in "$ROOT_DIR"/examples/*.cas "$BUILD_DIR/regress.cas"; do
    name=$(basename "$example" .cas)
    "$BUILD_DIR/commonasmc" "$example" --target nios2 -o "$BUILD_DIR/nios2-${name}.s"
    nios2-linux-gnu-as -o "$BUILD_DIR/nios2-${name}.o" "$BUILD_DIR/nios2-${name}.s"
  done
  echo "the Nios II assembler accepted every nios2 output."
else
  echo "no Nios II assembler found; skipped assembling the Nios II output."
fi
...
run_exec_everywhere() {
  ...
  run_exec_case superh      sh4-linux-gnu-as "" sh4-linux-gnu-ld "" qemu-sh4-static
